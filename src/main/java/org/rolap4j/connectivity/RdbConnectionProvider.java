...
/**
 * Use this <tt>Connection provider</tt> to execute classical SQL query.
 *
 ...
 */
@Slf4j
public class RdbConnectionProvider implements ConnectionProvider {
    ...
    public Connection getConnection() throws Rolap4jException {
        ...
        final DataSource src = Rolap4jConfig.getInstance().getDataSource();
        try {
            Class.forName(src.getDriver());
        } catch (ClassNotFoundException e) {
            throw new Rolap4jException(e.getMessage());
        }
        try {
            log.debug("Connecting to {} with the user {} ...", src.getUri(), src.getUsername());
            return DriverManager.getConnection(src.getUri(), src.getUsername(), src.getPassword());
